...
- **Zugang zu den geschützten Lehrpersonen-Seiten (seit 2026-08-11)** — Klassencode + Secret werden vom Hub in den `sessionStorage` gelegt (`src/lib/lehrperson/zugang.ts`), **nie** als Query-Parameter weitergereicht: sonst stünde das Secret in Verlauf, Lesezeichen und — beim Zeigen des Reports im Unterricht — an der Wand. `report/`, `setup/` und `admin/` benutzen dafür `useZugang()` + `<ZugangFormular>` aus `src/app/lehrperson/_components/ZugangGate.tsx` und zeigen bei fehlendem Zugang ein Eingabefeld statt einer Fehlermeldung. Alte Links mit `?code=&secret=` funktionieren weiter, werden aber sofort aus der Adresszeile entfernt.
